...
class InterceptSingleHandler<T> {
  final _InterceptSingleHandler _handler =
      _InterceptSingleHandler(intercepts: []);
...
// 定义拦截器类
class OneIntercept extends InterceptSingle<String> {
  @override
  void intercept(String data, SingleHandler handler) {
    print('one intercept $data');
    super.intercept(data, handler);
  }
}

class TwoIntercept extends InterceptSingle<String> {
  @override
  void intercept(String data, SingleHandler handler) {
    print('two intercept $data');
    super.intercept('modify by two $data', handler);
  }
}

void main() {
  var handler = InterceptSingleHandler<String>();
  handler.add(OneIntercept());
  handler.add(TwoIntercept());
  handler.intercept('hello world');
}
